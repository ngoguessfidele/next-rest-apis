...
export const PATCH = async (request: Request, context: {params: any}) => {
    ...
    try {
        ...
        return new NextResponse(
            JSON.stringify({ message: "Blog updated", blog: updatedBlog }), {status: 200}
        )

    } catch(error: any) {
        return new NextResponse("Error updating blog" + error.meassage)
    }
}
...
export const DELETE = async (request: Request, context: {params: any}) => {
    ...
    try {
        ...
        return new NextResponse(
            JSON.stringify({ message: "Error in deleting blog"}), { status: 500 }
        )
    }
}